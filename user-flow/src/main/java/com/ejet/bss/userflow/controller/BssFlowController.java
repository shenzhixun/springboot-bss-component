...
public class BssFlowController extends CoBaseController { 
	...
	public Result queryByPage(@RequestBody(required=true)Param param, BindingResult bindResult) {
		...
		try{
			...
			BssFlowModel model = toBean(param, new TypeReference<BssFlowModel>(){});
			PageBean<BssFlowModel> pageBean = mService.queryByPage(model, param.getPage().getPageNum(), param.getPage().getPageSize());
